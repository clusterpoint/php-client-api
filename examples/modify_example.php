...
//  $cpsConnection->setDebug(1);
  ...
  $insertRequest = new cps\CPS_InsertRequest(array('id2' => $document2, 'id3' => $document3));
  $cpsConnection->sendRequest($insertRequest);

  // Update
  $document['title'] = 'changed title';
  $updateRequest = new cps\CPS_UpdateRequest('id1', $document);
  $cpsConnection->sendRequest($updateRequest);
  $updateRequest = new cps\CPS_UpdateRequest(array('id1' => $document, 'id3' => $document3));
  $cpsConnection->sendRequest($updateRequest);

  // Replace
  $document['title'] = 'changed title for replace';
  $replaceRequest = new cps\CPS_ReplaceRequest('id1', $document);
  $cpsConnection->sendRequest($replaceRequest);
  $replaceRequest = new cps\CPS_ReplaceRequest(array('id1' => $document, 'id3' => $document3));
  $cpsConnection->sendRequest($replaceRequest);

  // Partial Replace
  $document = array('title' => 'original name');
  $replaceRequest = new cps\CPS_PartialReplaceRequest('id1', $document);
  $cpsConnection->sendRequest($replaceRequest);
  $replaceRequest = new cps\CPS_PartialReplaceRequest(array('id1' => $document, 'id3' => $document3));
  $cpsConnection->sendRequest($replaceRequest);
